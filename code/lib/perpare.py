...
import clip
import importlib
from lib.utils import choose_model,load_model_weights, MogLSTM
...
def prepare_models(args):
    ...
    NetG,NetD,NetC,CLIP_IMG_ENCODER,CLIP_TXT_ENCODER, NetD_DINO = choose_model(args.model)
    # image encoder
    ...
    # lstm models
    lstm = MogLSTM(512, 512, 5)  # 512 ViT-B/32

    # GAN models
    netG = NetG(args.nf, args.z_dim, lstm).to(device)
    netD = NetD(args.nf).to(device)
    netC = NetC(args.nf).to(device)
    NetD_DINO = NetD_DINO(512).to(device)  # ******GALIP OK
    ...
    if (args.multi_gpus) and (args.train):
        ...
        netC = torch.nn.parallel.DistributedDataParallel(netC, broadcast_buffers=False,
                                                          device_ids=[local_rank],
                                                          output_device=local_rank, find_unused_parameters=True)
        NetD_DINO = torch.nn.parallel.DistributedDataParallel(NetD_DINO, broadcast_buffers=False,
                                                          device_ids=[local_rank],
                                                          output_device=local_rank, find_unused_parameters=True)
    return CLIP_img_enc, CLIP_txt_enc, CLIP4trn, CLIP4evl, netG, netD, netC, NetD_DINO
...
#     #netC = NetC(args.nf, args.cond_dim, args.mixed_precision.to(device)).to(device)
#
#     netG = NetG(args.nf, args.z_dim).to(device)
#     netD = NetD(args.nf).to(device)
#     netC = NetC(args.nf).to(device)
#
#     if (args.multi_gpus) and (args.train):
...
#                                                           device_ids=[local_rank],
#                                                           output_device=local_rank, find_unused_parameters=True)
#     return image_encoder, text_encoder, CLIP4trn, CLIP4evl, netG, netD, netC
...
def prepare_dataloaders(args, transform=None):
    ...
    return train_dataloader, valid_dataloader, valid_dataloader, \
            train_dataset, valid_dataset, valid_dataset, train_sampler
