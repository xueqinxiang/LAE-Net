...
def main(args):
    ...
    train_dl, valid_dl, test_dl, train_dataset, valid_dataset, test_dataset, sampler = prepare_dataloaders(args)
    args.n_words = train_dataset.n_words
    # prepare models
    image_encoder, text_encoder, clip4trn, clip4evl, netG, netD, netC, netD_DINO = prepare_models(args)
    # prepare fixed data 
    #fixed_shape, fixed_sent, fixed_clip = get_fix_data(train_dl, test_dl, text_encoder, clip4trn, args)
    fixed_shape, fixed_sent, _, _ = get_fix_data(train_dl, test_dl, text_encoder, args)
    if (args.multi_gpus==True) and (get_rank() != 0):
        None
    else:
    ...
    D_params = list(netD.parameters()) + list(netC.parameters()) + list(netD_DINO.parameters())
    ...
    if args.state_epoch!=1:
        ...
        netG, netD, netC, netD_DINO, optimizerG, optimizerD = load_model_opt(netG, netD, netC, netD_DINO, optimizerG, optimizerD, path, args.multi_gpus)
    # print args
    if (args.multi_gpus==True) and (get_rank() != 0):
        None
    else:
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        train(train_dl, netG, netD, netC, netD_DINO, text_encoder, image_encoder,optimizerG, optimizerD, scaler_G, scaler_D, args)
        ...
        if epoch%gen_interval==0:
            ...
            sample(fixed_shape, fixed_txt, netG, args.multi_gpus, epoch, args.img_save_dir, writer)
            torch.cuda.empty_cache()
        # save models
        if epoch%save_interval==0:
            if (args.multi_gpus==True) and (get_rank() != 0):
                ...
                state = {'model': {'netG': netG.state_dict(), 'netD': netD.state_dict(), 'netC': netC.state_dict(), 'netD_DINO': netD_DINO.state_dict()}, \
        ...
        if epoch%test_interval==0:
            FID, IS, TI_score, L2_Err, MP = test(valid_dl, text_encoder, netG, clip4evl, args.device, m1, s1, epoch, args.max_epoch,
                                 args.sample_times, args.z_dim, args.batch_size, args.multi_gpus)
            torch.cuda.empty_cache()
            ...
            if (args.multi_gpus==True) and (get_rank() != 0):
                ...
                if epoch % test_interval == 0:
                    ...
                    print('The %d epoch CLIP_Score: %.2f, L2_Err: %.2f, MP: %.2f, FID: %.2f, IS: %.2f, '
                          % (epoch, TI_score * 100, L2_Err * 100, MP * 100, FID, IS))
                    eval_summary = open(os.path.join(log_dir, "metrics.txt"), "a")
                    eval_summary.write('Epoch:{} | FID:{:4f} | CLIP_Score:{:4f} \n'.format(epoch, FID, TI_score * 100))
                    eval_summary.close()
